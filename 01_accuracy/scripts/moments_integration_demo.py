...
Mirrored from ``pg_gpu/examples/moments_integration_demo.py``. The
upstream is the canonical example; this in-repo copy keeps the paper
figure regeneration tied to this repo's commit graph.
...
Produces:
  - 01_accuracy/figures/moments_3pop_true_model.png
  - 01_accuracy/figures/moments_3pop_fitted_vs_observed.png
  - 01_accuracy/tables/moments_3pop_integration_demo/  (scratch: VCFs, YAMLs, fitted parameters)

The ``moments`` package is heavy (msprime + demes + demesdraw + moments.LD)
and is not in the default pg_gpu pixi environment. Run inside the
``moments`` pixi feature instead:

    pixi run -e moments --manifest-path /home/adkern/pg_gpu/pixi.toml \\
        python 01_accuracy/scripts/moments_integration_demo.py
"""
...
# Scratch directory: VCF replicates, demes YAMLs (true + fitted),
# optimization options, and the Godambe-SE summary. Lives under tables/
# because none of these are figures; the two figures land in FIG_DIR.
DATA_DIR = "01_accuracy/tables/moments_3pop_integration_demo"
FIG_DIR = "01_accuracy/figures"
...
if __name__ == "__main__":
    ...
    logger.info(f"Scratch output will be saved to {DATA_DIR}")
    logger.info(f"Final figures will be saved to {FIG_DIR}")
    ...
    true_model_path = os.path.join(FIG_DIR, "moments_3pop_true_model.png")
    ...
    plot_path = os.path.join(FIG_DIR, "moments_3pop_fitted_vs_observed.png")
