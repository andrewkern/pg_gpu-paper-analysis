...
Recovered from ``pg_gpu/examples/multipop_ld_benchmark.py`` (last
present in pg_gpu at the parent of commit d0a2cf8, before PR #61's
examples cleanup). Produces the paper's per-population LD parity
figures.
...
Produces:
  - 01_accuracy/figures/moments_ld_{n_pops}pop.png

Needs the ``moments`` pixi feature (msprime + moments.LD).

Usage:
    pixi run -e moments --manifest-path /home/adkern/pg_gpu/pixi.toml \\
        python 01_accuracy/scripts/multipop_ld_benchmark.py 2
...
# Cache: VCFs + moments pickles persist across reruns so re-render of
# the plot doesn't pay the simulation + moments parsing cost twice.
CACHE_DIR = Path("01_accuracy/tables/multipop_ld_benchmark_cache")
OUT_DIR_FIG = Path("01_accuracy/figures")
...
def plot_results(mom, gpu, labels, stat_names, t_moments, t_pg, n_pops):
    ...
    outfile = OUT_DIR_FIG / f"moments_ld_{n_pops}pop.png"
