...
Recovered from ``pg_gpu/examples/four_pop_ld_demo.py`` (last present
in pg_gpu at the parent of commit d0a2cf8, before PR #61's examples
cleanup). Produces the paper's four-population LD validation figure.
...
Produces:
  - 01_accuracy/figures/moments_ld_validation.pdf
  - 01_accuracy/tables/four_pop_ld_demo_data/  (scratch: VCFs, samples, recombination map)

Needs the ``moments`` pixi feature.

Usage:
    pixi run -e moments --manifest-path /home/adkern/pg_gpu/pixi.toml \\
        python 01_accuracy/scripts/four_pop_ld_demo.py
"""
...
DATA_DIR = "01_accuracy/tables/four_pop_ld_demo_data"
...
def main():
    ...
    outfile = os.path.join(FIG_DIR, "moments_ld_validation.pdf")
