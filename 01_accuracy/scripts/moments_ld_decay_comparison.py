#!/usr/bin/env python
"""
Two-population LD-decay parity: pg_gpu vs moments inferred curves vs truth.

Recovered from an earlier version of
``pg_gpu/examples/moments_integration_demo.py`` (last present at the
parent of commit 1149612, before PR #60 simplified the demo into a
three-population admixture script). Produces the paper's 3x3 panel
LD-decay comparison figure.

Simulates 20 replicate 1Mb regions under an isolation-with-migration model
...
The figure overlays empirical bin-means with bootstrap SE, the inferred
LD-decay curve, and the true curve, for each of nine LD statistics
(DD, Dz, pi2 across the population pairings).
...
Produces:
  - 01_accuracy/figures/moments_ld_decay_comparison.pdf
  - 01_accuracy/tables/moments_ld_decay_comparison_data/  (scratch)

Needs the ``moments`` pixi feature.

Usage:
    pixi run -e moments --manifest-path /home/adkern/pg_gpu/pixi.toml \\
        python 01_accuracy/scripts/moments_ld_decay_comparison.py
"""
...
DATA_DIR = "01_accuracy/tables/moments_ld_decay_comparison_data"
...
def main():
    ...
    plot_ld_comparison(mv, opt_params, demo_func, R_BINS, g,
                       ...
                       output=os.path.join(FIG_DIR, "moments_ld_decay_comparison.pdf"))
